...
$setting_q="SELECT `turn_off_system` , `enable_register` 
FROM `settings`
WHERE `ID` =1";
//echo $setting_q;
$set_res=mysqli_query($link,$setting_q,1) or die("something went wrong ". mysqli_errno($link));
$set_row=mysqli_fetch_assoc($set_res);

                          if($set_row['turn_off_system']=='No'){?>
